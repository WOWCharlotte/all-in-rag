...
_dataset_dir = os.path.join(os.path.dirname(__file__), "evals", "datasets", "custom")
_loader = DirectoryLoader(_dataset_dir, glob="**/*.md")
_docs = _loader.load() if os.path.exists(_dataset_dir) else []
...
if __name__ == "__main__":
    ...
    base_url = os.getenv(
        "EVAL_LLM_BINDING_HOST", "https://api.deepseek.com"
    )
    api_key = os.getenv("EVAL_LLM_BINDING_API_KEY")
    if not api_key:
        print("Error: API key is not set.")
        print("Please set EVAL_LLM_BINDING_API_KEY or OPENAI_API_KEY.")
        exit(1)

    # Initialize RAG system with ChatOpenAI
    llm = ChatOpenAI(
        model=model,
        base_url=base_url,
        api_key=api_key,
    ...
    rag_client = ExampleRAG(llm_client=llm, retriever=r, logdir="evals/logs")
    ...
    query = "What is Ragas"
    print(f"Query: {query}")
    response = asyncio.run(rag_client.query(query, top_k=3))
